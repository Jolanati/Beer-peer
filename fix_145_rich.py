...
# ── Card text helpers ─────────────────────────────────────────────────────────
_STOP = {
    "a", "an", "the", "is", "are", "and", "or", "of", "with", "in", "on",
    "for", "its", "it", "this", "that", "has", "have", "very", "quite",
    "wine", "wines", "flavor", "flavors", "note", "notes", "also", "some",
}

def _food_char(desc: str, n: int = 2) -> str:
    """Pick n content words from a food description, joined with 'and'."""
    words = [
    ...
    return " ".join(chosen) if chosen else "rich and savory"
...
def _build_tier_text(r: dict, display_name: str, char: str) -> Text:
    ...
    t.append(f"As your {display_name} is {char},\n")
    t.append("we believe you need a wine that\n")
    t.append(f"{intent}", style="bold")
    t.append(" \u2014 something ")
    t.append(f"{adj},\n", style="bold italic")
    t.append("something that plays on\n")
    t.append(f"{kws}\n", style="bold cyan")
    t.append("\n")
...
def print_card(food_name: str, recs: list):
    ...
    char         = _food_char(entry.get("classic", ""), 2)
    ...
    # One panel per tier
    for r in recs:
        body = _build_tier_text(r, display_name, char)
        _console.print(Panel(
            body,
...
for cell in nb['cells']:
    if cell.get('id') == '5a710aff':
        ...
        changed += 1
        print('  ✓ Patched cell 5a710aff (§14.5)')
        break
...
if changed:
    ...
    print('WARNING: cell not found — nothing saved.')
